...
Tests use Node's test runner through `tsx` and must cover operation planning, FIFO ordering, validation, queue transitions, compensation, and recovery reports without requiring a live vault. `npm run check` is the same build, lint, and test gate used by CI. Manual Obsidian evidence complements, but does not replace, automated checks.
