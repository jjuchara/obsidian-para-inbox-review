...
Tests use Node's test runner through `tsx` and must cover operation planning, FIFO ordering, path validation, typed vault lookup, queue transitions, compensation, and recovery reports without loading the Obsidian runtime or requiring a live vault. `npm run check` is the same build, lint, and test gate used by CI. Manual Obsidian evidence complements, but does not replace, automated checks.
